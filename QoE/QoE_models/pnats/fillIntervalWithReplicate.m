function [Q_filled] = fillIntervalWithReplicate( Qr, r, ms, ls )
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if Ns == 0
        ...
        if ms(1) == 0
            Q_filled(1:(ms(1)+cls(1))) = padarray(Qr(1:(ms(1))),[fs(1) 0], 0, 'pre');
        else
            Q_filled(1:(ms(1)+cls(1))) = padarray(Qr(1:(ms(1))),[fs(1) 0], 'replicate', 'post');
        end
        
        for i = 2:Ns
            Q_filled((ms(i-1)+cls(i-1)+1):(ms(i)+cls(i))) = padarray(Qr((ms(i-1)+1:ms(i))),[fs(i) 0], 'replicate', 'post');
        end
